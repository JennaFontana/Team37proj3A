...
#define ECHO_GPIO GPIOC      // Example
#define ECHO_PIN  (1 << 13) // PC13
#define ECHO_PIN_NUMBER 13   // Pin number for EXTI configuration
...
void tim5_init(void) {
    ...
    TIM5->ARR = 49999; // Auto-reload for 50 ms at 16 MHz / 16
...
void GPIO_init(void) {
   ...
   // Configure TRIG_PIN (PA0) as output
   TRIG_GPIO->MODER &= ~(0x3 << (0 * 2)); // Clear mode bits for PA0
   TRIG_GPIO->MODER |= (0x1 << (0 * 2));  // Set PA0 as general purpose output
   TRIG_GPIO->OTYPER &= ~(1 << 0);        // Set PA0 as push-pull
   TRIG_GPIO->OSPEEDR |= (0x3 << (0 * 2)); // Set PA0 as high speed
   TRIG_GPIO->PUPDR &= ~(0x3 << (0 * 2)); // No pull-up, pull-down for PA0

   // Configure ECHO_PIN (PC13) as input
   ECHO_GPIO->MODER &= ~(0x3 << (ECHO_PIN_NUMBER * 2)); // Clear mode bits for PC13
   ECHO_GPIO->PUPDR &= ~(0x3 << (ECHO_PIN_NUMBER * 2)); // No pull-up, pull-down for PC13
